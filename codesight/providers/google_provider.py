...
class GoogleVertexProvider(BaseLLMProvider):
    """Google Cloud Vertex AI integration — Gemini 3.1 Pro, Gemini 3.1 Flash, etc."""
    ...
    def __init__(self, config: ProviderConfig) -> None:
        ...
        if not self._project:
            raise ValueError(
                ...
                "Set GOOGLE_CLOUD_PROJECT or configure it in ~/.codesight/config.json"
            )
    ...
    def _get_access_token(self) -> str:
        """Retrieve access token via Application Default Credentials."""
        try:
            import google.auth
